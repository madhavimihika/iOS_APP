...
struct LightItUpViewLevel2: View {
    ...
    private let litWindow = 1.2
    private let levelColor: Color = .blue
    private let levelName = "Level 2"
    ...
    var body: some View {
        ...
        .padding(.vertical)
        .navigationTitle("Light It Up L2")
    ...
    private var gameInfoView: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("15-30 seconds • 4 cards in a row")
                .font(.subheadline)
                .foregroundColor(.secondary)
            Text("• 1.2 seconds to tap the lit card")
                .font(.caption)
                .foregroundColor(.secondary)
            Text("• \(roundDuration)s round • Lives: \(lives)/\(maxLives)")
                .font(.caption)
                .foregroundColor(.secondary)
            Text("• Medium difficulty - stay focused!")
                .font(.caption)
                .foregroundColor(levelColor)
            ...
            if feedbackType != .none {
                HStack {
                    ...
                    Text(feedbackType == .correct ? "Correct! +1 " : "Wrong! -1 ")
                        .font(.caption)
                        .fontWeight(.bold)
                        .foregroundColor(feedbackType == .correct ? .green : .red)
                }
